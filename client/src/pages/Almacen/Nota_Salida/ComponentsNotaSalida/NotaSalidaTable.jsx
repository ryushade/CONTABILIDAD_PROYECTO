...
const TablaSalida = forwardRef(({ salidas }, ref)  => {
  ...
  const handleConfirmImprimir2 = () => {
    ...
    if (notSeleccionada) {
      generatePDF(notSeleccionada); // Genera el PDF solo para la guía seleccionada
    } else {
      toast.error("sexo")
    }
      ...
      <tr onClick={() => handleRowClick(salida.id)} className='tr-tabla-ingreso'>
        ...
        <td className='text-center'>
          <select className='b text-center custom-select border border-gray-300 rounded-lg text-gray-900 text-sm w-20' name="select" onClick={handleSelectClick} onChange={(e) => handleSelectChange2(e, salida.id)}>
            <option value="">...</option>
            <option value="imprimir2">PDF</option>
  ...
  return (
    <div className="container-table-reg px-4 bg-white rounded-lg">
      ...
      <table className="table w-full">
        <thead>
          <tr>
            ...
            <th className="w-1/12 text-center text-sm font-semibold text-gray-500 uppercase tracking-wider">DESTINATARIO</th>
      ...
      {isModalOpenImprimir2 && (
        <ConfirmationModal
          message="¿Desea guardar en PDF esta nota de salida?"
      ...
      {isModalOpenAnular && (
        <ConfirmationModal
          message="¿Desea anular esta nota de salida?"
