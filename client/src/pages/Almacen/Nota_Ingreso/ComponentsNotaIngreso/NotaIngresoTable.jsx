...
const TablaIngresos = ({ ingresos }) => {
  ...
  const generatePDFIngreso = (ingreso) => {
    ...
    <div class="p-5 text-sm leading-6 font-sans w-full">
        <div class="flex justify-between items-center mb-3">
            ...
            <div class="text-center border border-gray-400 rounded-md ml-8 overflow-hidden w-80">
                ...
                <h2 class="text-lg font-bold text-gray-800 p-2 border-b border-gray-400">${ingreso.documento}</h2>
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 gap-6 mb-6">
                <div class="space-y-2">
                    <p class="text-sm font-semibold text-gray-800">
                        <span class="font-bold text-gray-900">NRO. DOCU.:</span> <span class="font-semibold text-gray-600">${ingreso.documento}</span>
                    </p>
                    <p class="text-sm font-semibold text-gray-800">
                        <span class="font-bold text-gray-900">PROVEEDOR:</span> <span class="font-semibold text-gray-600">${ingreso.proveedor}</span>
                    </p>
                    <p class="text-sm font-semibold text-gray-800">
                        <span class="font-bold text-gray-900">ALMACÉN:</span> <span class="font-semibold text-gray-600">${ingreso.almacen_D}</span>
                    </p>
                    <p class="text-sm font-semibold text-gray-800">
                        <span class="font-bold text-gray-900">CONCEPTO:</span> <span class="font-semibold text-gray-600">${ingreso.concepto}</span>
                    </p>
                </div>
                <div class="space-y-2 ml-auto text-left">
                    <p class="text-sm font-semibold text-gray-800">
                        <span class="font-bold text-gray-900">FECHA:</span> <span class="font-semibold text-gray-600">${ingreso.fecha}</span>
                    </p>
                    <p class="text-sm font-semibold text-gray-800">
                        <span class="font-bold text-gray-900">USUARIO:</span> <span class="font-semibold text-gray-600">${ingreso.usuario}</span>
                    </p>
                </div>
            </div>
        </div>
  ...
  const options = {
    margin: [10, 10],
    filename: `${ingreso.documento}.pdf`,
  ...
  const handleConfirmImprimir2 = () => {
    const ingresoSeleccionada = ingresos.find((ingreso) => ingreso.id === notaIdToAnular); // Encuentra la guía seleccionada por ID
    if (ingresoSeleccionada) {
      generatePDFIngreso(ingresoSeleccionada); // Genera el PDF solo para la guía seleccionada
    }
    setIsModalOpenImprimir2(false); // Cierra el modal
  };
  ...
  const renderIngresoRow = (ingreso) => (
    <React.Fragment key={ingreso.id}>
      <tr onClick={() => handleRowClick(ingreso.id)} className='tr-tabla-ingreso'>
        ...
        <td className='text-center'>
          <select className='b text-center custom-select border border-gray-300 rounded-lg p-1.5 text-gray-900 text-sm' name="select" onChange={(e) => handleSelectChange2(e, ingreso.id)}>
            <option value="">...</option>
            <option value="imprimir2">Guardar PDF</option>
